...
def train(model, train_loader, epochs=10):
    ...
    for epoch in tqdm(range(1,epochs+1), desc="Training..."):
        ...
        for inputs, targets in train_loader:
            ...
            optimizer.zero_grad()
            output = model(inputs, 0)  # Memory = input for autoregressive training
            # print(output)
            output = output.view(-1, model_args.vocab_size)
...
end_time = time.time()
torch.save(model.state_dict(), f'./model_complete')

print(f"============== TRAINING DONE : time taken {end_time-start_time}========================")
...
def text_completion(model, prompts: list[str], temperature: float = 0.6, top_p: float = 0.9,
    ...
    for cur_pos in cur_iterator:
        with torch.no_grad():
            ...
            logits = model.forward(tokens[:, cur_pos - 1:cur_pos], cur_pos - 1)
